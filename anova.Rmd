...
for (sd in sds) {
  ...
  power_samples <- pwr.anova.test(k=10,f=20/sd,sig.level=.1,power=.8)
  print(power_samples)
  
  print(str_interp("samples needed to detect difference of 20s between groups with SD ${sd} at sig level .1 and power .8: "))
  power_samples <- pwr.anova.test(k=3,f=20/sd,sig.level=.1,power=.8)
  print(power_samples)
}
